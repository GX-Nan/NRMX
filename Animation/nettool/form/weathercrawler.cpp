...
void WeatherCrawler::Analysis(QStringList data,int Key)
{
    ...
    else if(Key>=2&&Key<=5)
    {
        ...
        for(int i=11;i<=12;i++)
        {
            Time.append(data[0].at(i));
        }
        Message->HoulyTemp.insert(Time,data[2]);
        Message->HoulyWeather.insert(Time,data[1]);
        Message->HoulyHum.insert(Time,data[3]);
        qDebug()<<Key;
        if(Key==17){
...
void WeatherCrawler::Crawl()
{
    ...
    qDebug()<<"step----5";
    PyObject * pReturn=PyEval_CallObject(Crawler,ArgList);//调用函数,并传入参数pArgs
    qDebug()<<"step----6";
    QStringList list1;
