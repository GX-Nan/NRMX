...
//写一个工作执行寄存的程序----1.先执行看是否执行完了2.执行完了之后把数据----例子：温度 25 或者是开关：True这样记录下来3.3000条的话就清空，重新写入;3.如果出现错误则放入另一个text内-->并且上报到云服务器上
    ...
    if(Judge==false){
        for(int i=0;i<(buffer.length())/10;i++){
            ...
            {
                StopReturn=1;
                if(Transition.at(4)=="1")
                {
                    Analysis_Window(Transition);
                }
                else if(Transition.at(4)=="2")
                {
                    Analysis_Curtains(Transition);
                }
            }
        }
    }
...
void frmTcpClient::Analysis_Curtains(QString Data)
{
    QString Sub;
    QString Value;
    QString Function;
    ...
    emit Updata_Curtain(Sub.toInt(),Value.toInt());
}

void frmTcpClient::Analysis_Window(QString Data)
{
    QString Sub;
    QString Value;
    QString Function;
    ...
    emit Updata_Window(Sub.toInt(),Value.toInt());
}

void frmTcpClient::RecoverSignals()
{
    StopReturn=0;
}
