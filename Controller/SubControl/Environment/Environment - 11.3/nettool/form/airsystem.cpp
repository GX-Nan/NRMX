...
void AirSystem::ReceiveData(int Value)//从分控接受后--界面更新
{ 
    emit StartSignal();
    qDebug()<<"Air---station";
    if(Value<16){
...
void AirSystem::SwitchButton_U_D(int Flag)
{
    if(Flag==1)
    {
        SetInstruction(7);
...
void AirSystem::SwitchButton_L_R(int Flag)
{
    if(Flag==1)
    {
        SetInstruction(5);
...
void AirSystem::Trigger_TempQslider()
{
    ...
    emit AirTemp_Sync(SetTemp.toInt());
...
void AirSystem::Main_AirTemp(int value)
{
    ui->TempQslider->setValue(value);
    Trigger_TempQslider();
}

void AirSystem::Main_AirSwitch(int value)
{
    switch (value) {
    case 0:
        if(Falg_AirSwitch==0){
