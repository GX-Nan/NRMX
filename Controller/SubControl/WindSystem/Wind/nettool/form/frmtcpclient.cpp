...
void frmTcpClient::readData()//用信号槽连接主线程ui控件的修改-----要不要放进线程内处理线程
{
    ...
    QString Transition;
    QString AirTrasition;
    if(Judge==false){
        for(int i=0;i<(buffer.length())/10;i++){
            for(int a=0;a<10;a++){
                Transition.append(data.at(i*10+a));
            }
            if(Transition.at(2)=="6")
            {
                Analysis_Wind(Transition);
            }
            else if(Transition.at(2)=="3")
            {
                for(int i=7;i<=8;i++)
                {
                    AirTrasition.append(Transition.at(i));
                }
                qDebug()<<"Trasition:"<<AirTrasition;
                if(Transition.at(5)=="0"){
                    if(Transition.at(6)=="1")
                    {
                        qDebug()<<"Temp:"<<AirTrasition;
                        emit OutSideTemp(AirTrasition.toInt());
                    }
                    else if(Transition.at(6)=="2")
                    {
                        emit OutSideHum(AirTrasition.toInt());
                    }
                    else if(Transition.at(6)=="3")
                    {
                        emit OutSideNo2(AirTrasition.toInt());
                    }
                    else if(Transition.at(6)=="4")
                    {
                        emit OutSidePm25(AirTrasition.toInt());
                    }
                    else if(Transition.at(6)=="5")
                    {
                        emit OutSideSo2(AirTrasition.toInt());
                    }
                    else if(Transition.at(6)=="7")
                    {
                        emit OutSidePm10(AirTrasition.toInt());
                    }
                }
                else {
                    if(Transition.at(6)=="1")
                    {
                        emit OutSideAQI(AirTrasition.toInt());
                    }
                }
                Transition.clear();
                AirTrasition.clear();
...
void frmTcpClient::Analysis_Wind(QString Data)
{
    QString Sub;
    QString Value;
    QString Function;
