...
//            m_serialPort->open(QIODevice::op);
...
// 打开串口
bool Serial::open(QString serialName, int baudRate=9600)
{
    // 设置串口名
...
//    m_serialPort->open(QIODevice::ReadWrite);
    ...
    if(m_serialPort->open(QIODevice::ReadWrite))
    {
        ...
        m_serialPort->setBaudRate(baudRate); // 设置波特率9600
        ...
        m_serialPort->setFlowControl( QSerialPort::NoFlowControl ); // 设置流控制(无数据流控制)
          // m_serialPort->open(QIODevice::open());
...
{
    // 将下位机发来数据存储在数据缓冲区
    m_readBuf = m_serialPort->readAll();//Get data from serialport,is a QByteArray
...
// 将16进制字符串转换为对应的字节序列
QByteArray Serial::hexStringToByteArray(QString HexString)//Used to send message
{
    bool ok;
    QByteArray ret;
