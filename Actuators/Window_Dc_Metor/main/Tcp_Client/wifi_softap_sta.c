...
/*配置模组自身的无线热�?*/
#define ESP_WIFI_AP_SSID       "Spider"  //设置模块发出的无线名�?
#define ESP_WIFI_AP_PASS       "12356789"    //无线密码
#define ESP_WIFI_AP_CHANNEL    11            //信道(默认1)
#define ESP_WIFI_AP_MAX_COUNT  5             //最大连接数(最�?10�?)


/*配置模组连接的路由器*/
// #define ESP_WIFI_STA_SSID       "Raspberry Pi 4B"  //设置模块连接的无线名�?
// #define ESP_WIFI_STA_PASS       "qwerty123567"    //无线密码
#define ESP_WIFI_STA_SSID       "Fs"  //设置模块连接的无线名�?
...
void wifi_init_softap_sta(void)
{
    ...
    esp_netif_create_default_wifi_ap();//创建�? TCP/IP 堆栈的默认网络接口实例绑定AP�?
    esp_netif_create_default_wifi_sta();//创建�? TCP/IP 堆栈的默认网络接口实例绑定STA�?

    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));//创建 Wi-Fi 驱动程序任务，并初始�? Wi-Fi 驱动程序�?
   ...
   // ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));//设置AP+STA模式
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));//设置AP+STA模式
